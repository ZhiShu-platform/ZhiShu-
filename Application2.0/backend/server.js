...
const pool = new Pool({
  user: process.env.DB_USER || 'zs_zzr',
  host: process.env.DB_HOST || 'localhost',
  database: process.env.DB_NAME || 'zs_data',
  password: process.env.DB_PASSWORD || '373291Moon',
...
let mcpServices = [
  ...
  {
    name: 'amap_mcp',
    display_name: '高德地图MCP服务',
    status: 'running',
    type: 'http',
    host: process.env.AMAP_MCP_URL || 'https://mcp.amap.com',
    port: 443,
    error_message: null,
    pid: null,
    category: 'geospatial',
    api_key_required: true,
    tools: ['地理编码', '逆地理编码', '路径规划', '天气查询']
  },
  // 新增：Atlas GIS MCP服务
  {
    name: 'atlas_gis_mcp',
    display_name: 'Atlas GIS MCP服务',
    status: 'running',
    type: 'http',
    host: process.env.ATLAS_MCP_URL || 'http://localhost:3001',
    port: 3001,
    error_message: null,
    pid: null,
    category: 'geospatial',
    api_key_required: true,
    tools: ['国际化地理编码', '国际化路径规划']
  },
  // 新增：扩展MCP服务管理器
  {
    name: 'extended_mcp_manager',
    display_name: '扩展MCP服务管理器',
    status: 'running',
    type: 'http',
    host: 'localhost',
    port: 8000,
...
// API密钥配置接口
app.post('/api/mcp/services/configure', (req, res) => {
  try {
    ...
    // 更新服务配置
    if (service.name === 'amap_mcp') {
      process.env.AMAP_API_KEY = api_key;
      console.log(`✅ 高德地图API密钥已配置`);
    } else if (service.name === 'atlas_gis_mcp') {
      process.env.ATLAS_TOKEN = api_key;
      console.log(`✅ Atlas GIS Token已配置`);
    }
    ...
    res.json({
      success: true,
      message: `${service.display_name} API密钥配置成功`,
      service: {
        name: service.name,
